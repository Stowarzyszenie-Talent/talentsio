$(function() {
    ...
    function synchronizeTime(data) {
        ...
        const phaseStartDate = data.phase_start_date
        const phaseEndDate = data.phase_end_date
        const phaseRoundName = data.phase_round_name
        const phaseMultiplier = data.phase_multiplier
        ...
        if (phaseEndDate && (!countdownDate || phaseEndDate < countdownDate)) {
            countdownDate = phaseEndDate
            roundDuration = phaseEndDate - phaseStartDate
            roundName = gettext("phase x%(multiplier)s in %(round)s").fmt({
                multiplier: phaseMultiplier,
                round: phaseRoundName
            });
        }
        else {
